...
In `{epiparameter}`, you will mostly find **continuous** distributions like these. You can use `plot()` to `<epidist>` objects to visualise: 
...
But, how can we use `{epiparameter}` to access to specific distribution values for data analysis tasks?
