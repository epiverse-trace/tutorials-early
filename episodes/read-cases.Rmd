...
case_data <- rio::import(here::here("data", "ebola_cases.csv"))
head(case_data, 5)
```
...
You can check the [full list of supported file formats](http://gesistsa.github.io/rio/#supported-file-formats) in the `{rio}` package on the package website. Here is a selection of some key ones:
...
rio:::rio_formats %>% 
  ...
  filter(
    magrittr::is_in(
      input,
      c("epiinfo","excel","zip","parquet","json","googlesheets")
      )
    ) %>% 
  select(
    "Name" = format_name, 
    ...
    import_function) %>% 
  mutate(
    import_function = str_replace(import_function,"(.+)::(.+)","\\1"),
    "Import package" = str_replace(import_function, "(.+)","\\{\\1\\}")
  ) %>% 
  select(-import_function) %>% 
  knitr::kable()
`` ` (FIX THIS chunk closure)

Access to all the available formats run:
...
rio::import(file.path("path", "file_name.zip", fsep = "/"))
```
...
This code first establishes a connection to an SQLite database created in memory using `dbConnect()`. Then, it writes the `case_data` into a table named 'cases' within the database using the `dbWriteTable()` function. Subsequently, it reads the data from the 'cases' table using `dbReadTable()`. Finally, it closes the database connection with `dbDisconnect()`. More examples about SQL databases and R can be found [here](https://datacarpentry.org/R-ecology-lesson/05-r-and-databases.html).

:::::::::::::::::::::: callout

### Run SQL queries in R using dbplyr

Using a database interface package is invaluable for optimizing memory usage because it can strategically limit the amount of data pulled from the database through queries. By leveraging such a package, one can ensure that most aggregation tasks are performed directly within the database before extracting the necessary information. This approach minimizes the need to load large datasets into local memory for processing, thereby conserving resources and enhancing performance. Conversely, relying solely on external data manipulation processes can lead to suboptimal memory usage. Conducting all data operations outside the database could tax system resources and hinder efficiency.
