...
Each entry in the dataset represents a subject (e.g. a disease case or study participant) and should be distinguishable by a specific ID formatted in a particular way.
...
#### How to fix duplicated subject IDs?
...
dat_without_missings_dates <- sim_ebola_data[1:100, ] %>%
...
> Note that we check for a subset of 100 rows using `sim_ebola_data[1:100, ]`. The whole data frame contains more than 600 incorrect date sequences. Try it out yourself!
...
We can describe this delay using visualizations:
...
We can also use **summary statistics** or **probability distribution** parameters to describe different delays. You can review these concepts with [two episodes introducing delays for outbreak data](https://epiverse-trace.github.io/tutorials/):
...
cleaned_data <- raw_ebola_data %>%
  ...
  # epidemiological operations (with dates and data dictionary)
  cleanepi::standardize_dates(
    target_columns = c("date_onset", "date_sample")
  ) %>%
  cleanepi::check_date_sequence(
