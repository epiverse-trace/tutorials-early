...
We will use the following R packages to clean this dataset:

* `{here}` for easy file referencing,
* `{rio}` to import the data into R,
* `{cleanepi}` to clean it,
* `{dplyr}` to perform some data processing operations,
* `{magrittr}` to use its **pipe operator (`%>%`)**.

We encourage users with recent versions of R (version > 4.2) to use the base R
