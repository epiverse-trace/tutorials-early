...
# PERFORM THE OPERATIONS USING THE base R SYNTAX
ops <- list(
  standardize_column_names = list(keep = "V1", rename = NULL),
...
Further more you can compine multiple data cleaning tasks via the pipe operator in "|>", as shown in the below code snippet. 
```{r}
# PERFORM THE OPERATIONS USING THE pipe SYNTAX
cleaned_data <- raw_ebola_data |>
  cleanepi::standardize_column_names(keep = NULL, rename = NULL) |>
  ...
  cleanepi::standardize_dates(target_columns = c("date_onset", "date_sample"),
                              ...
                              format          = NULL,
                              timeframe       = NULL) |>
  cleanepi::check_subject_ids(target_columns = "case_id",
                              range          = c(1, 15000)) |>
