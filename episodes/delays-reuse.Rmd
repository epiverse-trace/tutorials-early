...
Let's start loading the `{epiparameter}` package. We'll use the pipe `%>%` to connect some of their functions, so let's also call to the `{magrittr}` package:
...
With this query combination, we get more than one delay distribution. This output is an object class `<multi_epidist>`.
...
To summarise a `<multi_epidist>` view and get the column names from the underlying parameter database, we can add the `epiparameter::list_distributions()` function to the previous code using the pipe `%>%`:
...
```{r,eval=FALSE}
# assign <multi_epidist> to object
distribution <-
  epiparameter::epidist_db(
    disease = "covid",
    epi_dist = "serial"
