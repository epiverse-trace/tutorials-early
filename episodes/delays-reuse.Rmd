...
Notice that with these pieces of information we can replace one out of three inputs of the `EpiNow2::dist_spec()` function:
...
In coming episodes we'll learn how to use `{EpiNow2}` to estimate transmissibility and how to use **distribution functions** to get a maximum value (`max`) for `EpiNow2::dist_spec()`.
