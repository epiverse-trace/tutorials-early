...
- To analyze data by person, place, and time:
    ...
    - Compare patterns across locations and demographic groups (e.g., age, sex).
...
We can describe epicurves by comparing the trend of new cases in time between demographic groups. We can compare features like:

- Size of peak or plateau,
- Time to peak (if any),
- Growth rate.

For example, in the figure below, we have two epidemic curves for the same outbreak stratified by sex. In the population, most of cases were observed in females.

- The size of peak in females was of ~70 incident cases; in males this was of about ~22 incidence cases.
- The time to peak in females happened around epiweek 15; in males this was around epiweek 20.
- The growth rate in females may be higher than males.
...
# Cases appear between exposure_start + min_incubation and exposure_end + max_incubation
...
ggplot(cases, aes(x = day, y = count)) +
  ...
  # Exposure window
  annotate("rect", xmin = exposure_start, xmax = exposure_end,
           ymin = 0, ymax = Inf, alpha = 0.15, fill = "red") +
  annotate("text", x = (exposure_start + exposure_end) / 2, y = 4.8,
           label = "Exposure\nWindow", color = "red", size = 3.5, fontface = "bold") +

  # First case line and label
  geom_vline(xintercept = first_case, linetype = "dashed", color = "darkgreen", linewidth = 0.8) +
  annotate("text", x = first_case + 0.15, y = 5.5,
           label = "First case", color = "darkgreen", size = 3.2, hjust = 0) +

  # Last case line and label
  geom_vline(xintercept = last_case, linetype = "dashed", color = "purple", linewidth = 0.8) +
  annotate("text", x = last_case - 0.15, y = 5.5,
           label = "Last case", color = "purple", size = 3.2, hjust = 1) +

  # Min incubation bracket (above bars)
  annotate("segment", x = exposure_start, xend = first_case,
           y = 6.8, yend = 6.8, color = "darkgreen",
           arrow = arrow(ends = "both", length = unit(0.2, "cm"))) +
  annotate("text", x = (exposure_start + first_case) / 2, y = 7.1,
           label = "Min incubation period", color = "darkgreen", size = 3) +

  # Max incubation bracket (above bars)
  annotate("segment", x = exposure_end, xend = last_case,
           y = 7.5, yend = 7.5, color = "purple",
           arrow = arrow(ends = "both", length = unit(0.2, "cm"))) +
  annotate("text", x = (exposure_end + last_case) / 2, y = 7.8,
           label = "Max incubation period", color = "purple", size = 3) +

  scale_x_continuous(breaks = 0:10, name = "Day") +
  scale_y_continuous(name = "Number of Cases", limits = c(0, 8.2),
                     breaks = scales::breaks_pretty()) +
  labs(
    title = "Epidemic Curve: Estimating Exposure from Incubation Period",
    subtitle = "Working backwards from first and last cases to identify the exposure window"
  ) +
  theme_classic() +
  theme(
