...
From a maximum value with `quantile()`, we can create a sequence of quantile values as a numeric vector and map density values for each:
...
<!-- evaluate to keep or diversity assessments using PMF -->
...
# we can get the PMF 
# but also the extract_param

# problem
# the summary statistics do not have mean and sd
influenza_generation$summary_stats

# get parameters (new function!)
influenza_generation_parameters <- 
...
influenza_converted <- 
  convert_params_to_summary_stats(
    ...
    shape = influenza_generation_parameters["shape"],
    scale = influenza_generation_parameters["scale"]
  )
...
influenza_generation_pmf <- 
  ...
  mutate(
    ...
    # for each quantile in the density function
    density_values =
      density(covid_serialint_discrete, at = quantile_values)
  )
