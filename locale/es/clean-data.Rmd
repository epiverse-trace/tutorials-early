...
- Descargar el [ébola\_simulada\_2.csv](https://epiverse-trace.github.io/tutorials-early/data/simulated_ebola_2.csv)
...
En el proceso de análisis de datos sobre brotes, es esencial asegurarse de que el conjunto de datos está limpio, curado y estandarizado,
y válido para facilitar un análisis preciso y reproducible.
Este episodio se centra en la limpieza de los datos de epidemias y brotes epidémicos mediante el programa
[cleanepi](https://epiverse-trace.github.io/cleanepi/) paquete,
...
para limpiarlos. Utilizaremos la tubería `%>%` para conectar algunas de sus funciones, incluidas otras de
...
descritas en el [Leer datos del caso](../episodes/read-cases.Rmd) episodio. Esto implica cargar
el conjunto de datos en `R` entorno y ver su estructura y contenido.
...
Vamos a **diagnosticar** el marco de datos. Enumera todas las características del marco de datos anterior que son problemáticas para los datos
análisis de datos.
...
Media un breve debate para relacionar la característica diagnosticada con las operaciones de limpieza necesarias.

Puedes utilizar estos términos para **diagnosticar características**:

- *Codificación*, como entradas de sexo y edad utilizando números, letras y palabras. También fechas en diferente disposición
  ("dd/mm/aaaa" o "aaaa/mm/dd") y formatos. Menos visibles, pero también los nombres de las columnas.
- *Falta* ¿cómo interpretar una entrada como "" en estado o "-99" en otra columna? ¿tenemos un diccionario de datos de
  el proceso de recogida de datos?
- *Inconsistencias* como tener una fecha de muestra anterior a la fecha de inicio.
- *Valores no plausibles* como observaciones atípicas con fechas fuera de un marco temporal previsto.
...
- Comprueba la secuencia de eventos fechados

::::::::::::::::::::::::::::::

## Una inspección rápida

La exploración e inspección rápidas del conjunto de datos son cruciales antes de sumergirse en cualquier tarea de análisis. En `{cleanepi}`
simplifica este proceso con la función `scan_data()` función Veamos cómo puedes utilizarla:

```{r}
cleanepi::scan_data(raw_ebola_data)
```

Los resultados ofrecen una visión general del contenido de cada columna, incluidos los nombres de las columnas y el porcentaje de algunos datos
...
Si quieres mantener ciertos nombres de columna sin someterlos al proceso de normalización, puedes utilizar
la dirección `keep` argumento de la función `cleanepi::standardize_column_names()`. Este argumento acepta un vector de columna
...
- Normaliza los nombres de las columnas del conjunto de datos de entrada, pero mantén los nombres de la primera columna tal cual.
...
Los datos brutos pueden contener irregularidades como **duplicados** filas, **vacío** filas y columnas, o **constante** columnas
(donde todas las entradas tienen el mismo valor.) Funciones de `{cleanepi}` como `remove_duplicates()` y `remove_constants()`
eliminar esas irregularidades, como se demuestra en el fragmento de código siguiente.
...
Duplicados se refiere sobre todo a filas replicadas. Las filas o columnas vacías pueden ser un subconjunto dentro del conjunto de filas constantes
o columnas.
...
Observa a los alumnos que el usuario puede crear nuevas columnas o filas constantes tras eliminar algunas iniciales.
...
Además de las regularidades, los datos brutos pueden contener valores perdidos que pueden estar codificados por diferentes cadenas,
incluidas las vacías. Para garantizar un análisis sólido, es una buena práctica sustituir todos los valores perdidos por `NA` en el
...
En el conjunto de datos sin procesar, algunas columnas pueden contener una mezcla de valores numéricos y de caracteres, y querrás convertirlos en valores numéricos.
valores de caracteres explícitamente en numéricos. Por ejemplo, en nuestro conjunto de datos simulado, en la columna edad algunas entradas son
escritas con palabras. En `{cleanepi}` la función `convert_to_numeric()` realiza dicha conversión como se ilustra a continuación
fragmento de código.
...
Gracias al `{numberize}` paquete, podemos convertir números escritos como palabras en inglés, francés o español a positivo
¡enteros positivos!
...
Además de las tareas habituales de limpieza de datos, como las comentadas en la sección anterior, el `{cleanepi}` paquete ofrece
...
### Comprobación de la secuencia de eventos fechados

Garantizar el orden y la secuencia correctos de los eventos fechados es crucial en el análisis de datos epidemiológicos, especialmente
cuando se analizan enfermedades infecciosas, en las que la cronología de acontecimientos como la aparición de síntomas y la recogida de muestras es esencial.
...
Puedes iniciar un diccionario personalizado con un marco de datos dentro o fuera de R. Puedes utilizar la función
...
la recogida de muestras y el análisis (es decir, la diferencia de tiempo entre hoy y la recogida de muestras). Lo más habitual
es calcular la edad de todos los sujetos a partir de su fecha de nacimiento (es decir, la diferencia de tiempo entre el día de hoy y la recogida de muestras).
y la fecha de nacimiento).

En `{cleanepi}` ofrece una cómoda función para calcular el tiempo transcurrido entre dos acontecimientos fechados en
...
de recogida de la muestra para cada caso, medido en años, y el tiempo restante medido en meses.
...
- reemplazar las cadenas missing as por una entrada missing válida
...
`Hmisc::cut2(x = age_in_years,cuts = c(20,35,60))` que da calcular el valor máximo y no requiere más
argumentos.
...
Realizar las operaciones de limpieza de datos individualmente puede llevar mucho tiempo y ser propenso a errores. El sitio `{cleanepi}` paquete
simplifica este proceso ofreciendo una cómoda función envolvente llamada `clean_data()` que te permite realizar
varias operaciones a la vez.

La página `clean_data()` aplica una serie de operaciones predefinidas de limpieza de datos al conjunto de datos de entrada. Aquí tienes una
...
Además, puedes combinar varias tareas de limpieza de datos mediante el operador de tubería "%>%", como se muestra en el código siguiente
fragmento.
...
En `{cleanepi}` El paquete genera un informe completo en el que se detallan las conclusiones y acciones de toda la limpieza de datos
...
- Utiliza `{cleanepi}` para limpiar y normalizar los datos de epidemias y brotes epidémicos
